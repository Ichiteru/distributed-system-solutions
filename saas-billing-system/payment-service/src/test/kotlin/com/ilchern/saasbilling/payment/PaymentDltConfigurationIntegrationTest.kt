...
class PaymentDltConfigurationIntegrationTest(
  ...
  @Value("\${payment.kafka.topics.commands-dlt.name}") private val dltTopicName: String,
  @Value("\${payment.kafka.dlt.retry.backoff-ms}") private val dltRetryBackoffMs: Long,
  @Value("\${payment.kafka.dlt.retry.max-attempts}") private val dltRetryMaxAttempts: Long,
) {

  @Test
  fun `payment commands listener factory is wired with DLT error handler`() {
    val factoryErrorHandler = DirectFieldAccessor(listenerContainerFactory)
      .getPropertyValue("commonErrorHandler")

    assertThat(dltTopicName).isEqualTo("payment.commands.dlt")
  ...
  companion object {
    @Container
    @JvmStatic
    private val postgres = PaymentDltPostgreSQLContainer("postgres:16-alpine")
    ...
    fun registerProperties(registry: DynamicPropertyRegistry) {
      ...
      registry.add("SPRING.KAFKA.PROPERTIES.SCHEMA.REGISTRY.URL") { "mock://payment-dlt-test" }
      registry.add("PAYMENT.PROVIDER.BASE_URL") { "http://127.0.0.1:65535" }
    }
  }
}

private class PaymentDltPostgreSQLContainer(
  imageName: String,
) : PostgreSQLContainer<PaymentDltPostgreSQLContainer>(imageName)
