...
class BillingInfrastructureConfig {
  ...
  @Bean
  fun billingCommandDltProducerFactory(
    kafkaProperties: KafkaProperties,
  ...
  @Bean
  fun billingCommandDltKafkaTemplate(
    billingCommandDltProducerFactory: ProducerFactory<String, GenericRecord>,
  ): KafkaTemplate<String, GenericRecord> =
    KafkaTemplate(billingCommandDltProducerFactory)

  @Bean
  fun billingCommandErrorHandler(
    billingCommandDltKafkaTemplate: KafkaTemplate<String, GenericRecord>,
    @Value("\${billing.kafka.topics.commands-dlt.name}") billingCommandDltTopicName: String,
    @Value("\${billing.kafka.dlt.retry.backoff-ms}") dltRetryBackoffMs: Long,
    @Value("\${billing.kafka.dlt.retry.max-attempts}") dltRetryMaxAttempts: Long,
  ): DefaultErrorHandler {
    val recoverer = DeadLetterPublishingRecoverer(billingCommandDltKafkaTemplate) { record, _ ->
      TopicPartition(billingCommandDltTopicName, record.partition())
    }
  ...
  fun billingCommandKafkaListenerContainerFactory(
    ...
    kafkaProperties: KafkaProperties,
    billingCommandErrorHandler: DefaultErrorHandler,
    ...
    ConcurrentKafkaListenerContainerFactory<String, GenericRecord>().apply {
      ...
      setCommonErrorHandler(billingCommandErrorHandler)
