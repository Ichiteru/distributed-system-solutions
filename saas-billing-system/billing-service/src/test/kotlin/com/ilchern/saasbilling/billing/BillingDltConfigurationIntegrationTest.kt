...
class BillingDltConfigurationIntegrationTest(
  ...
  @Value("\${billing.kafka.topics.commands-dlt.name}") private val dltTopicName: String,
  @Value("\${billing.kafka.dlt.retry.backoff-ms}") private val dltRetryBackoffMs: Long,
  @Value("\${billing.kafka.dlt.retry.max-attempts}") private val dltRetryMaxAttempts: Long,
) {

  @Test
  fun `billing commands listener factory is wired with DLT error handler`() {
    val factoryErrorHandler = DirectFieldAccessor(listenerContainerFactory)
      .getPropertyValue("commonErrorHandler")

    assertThat(dltTopicName).isEqualTo("billing.commands.dlt")
  ...
  companion object {
    @Container
    @JvmStatic
    private val postgres = BillingDltPostgreSQLContainer("postgres:16-alpine")
    ...
    fun registerProperties(registry: DynamicPropertyRegistry) {
      ...
      registry.add("SPRING.KAFKA.PROPERTIES.SCHEMA.REGISTRY.URL") { "mock://billing-dlt-test" }
    }
  }
}

private class BillingDltPostgreSQLContainer(
  imageName: String,
) : PostgreSQLContainer<BillingDltPostgreSQLContainer>(imageName)
